-- Fix for book_class function NULL reference bug
-- This patch fixes the issue where accessing class_schedule_record.class_id
-- before checking if class_schedule_record is NULL causes a system error
...
BEGIN
  ...
  RETURN jsonb_build_object(
    ...
    'updated_subscription', jsonb_build_object(
      'credits_remaining', subscription_record.credits_remaining - CASE WHEN subscription_record.plan_type = 'abonnement' THEN 0 ELSE 1 END,
      'weekly_credits_used', subscription_record.weekly_credits_used + CASE WHEN subscription_record.plan_type = 'abonnement' THEN 1 ELSE 0 END,
      'credits_used', CASE WHEN subscription_record.plan_type = 'abonnement' THEN NULL ELSE (COALESCE((SELECT credits_used FROM public.user_subscriptions WHERE id = subscription_record.id), 0) + 1) END
    )
  );
